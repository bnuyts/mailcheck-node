    ...
    async request(path, init) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new error_js_1.MailCheckError(res.status, body.code ?? body.error ?? 'unknown_error', body.message);
            }
            return (await res.json());
        }
        catch (err) {
            if (err instanceof error_js_1.MailCheckError)
                throw err;
            if (err.name === 'AbortError') {
                throw new error_js_1.MailCheckError(0, 'timeout', `Request to ${path} timed out after ${this.timeout}ms`);
            }
            throw err;
        }
        finally {
